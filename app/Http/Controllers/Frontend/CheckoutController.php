...
class CheckoutController extends Controller
{
    ...
    public function checkoutOrder(Request $request)
    {
        if (Auth::check()) {
            $request->validate([
                ...
                'shipping_name.required' => __('The name field is required.'),
                'shipping_street_address.required' => __('The address field is required.'),
                'shipping_state.required' => __('The state field is required.'),
                'shipping_City.required' => __('The City field is required.'),
                'shipping_country.required' => __('The country field is required.'),
            ]);

            $user_id = Auth::id();
            $user = Auth::user();
            if ($user->is_admin) {
                return redirect()->back()->with('error', __('Please sign in as a customer'));
            }
            try {
                $subtotal = Cart::subtotal();
                $tax = tax_amount($subtotal);
                $shipping_charge = delivery_charge($request->shipping_City);
                ...
                session()->put('tax', $tax);
                return $this->orderCreateCall($order_number, $shipping_charge, $tax, $subtotal, $this->discount, $this->grand_total, $request->payment);
            } catch (\Exception $e) {
                dd($e);
            ...
            return redirect()->back()->with('error', __('Please sign in'));
        }
    }
    ...
    public function orderCreateCall($order_number, $shipping_charge, $tax, $subtotal, $discount, $grand_total, $payment_method, $txn = null)
    {
        ...
        if ($order['success'] == true) {
            if ($payment_method == 'creditcard') {
                ...
                $smartLifeGrandTotal = $grand_total - $discount;
                $smartLifeDiscount = $discount;
                $data = [
                    ...
                    'metadata' => [
                        ...
                        'smartLifeDiscount' =>   $smartLifeDiscount,
                        'smartLifeGrandTotal' =>   $smartLifeGrandTotal,
                        'tax' =>   $tax,
                ...
                try {
                    ...
                    Session::put('payment_session_id', $session_id);
                    return redirect($client->getPayUrl($session_id));
                } catch (\Exception $e) {
                    dd($e);
                }
                ...
                Cart::destroy();
                return redirect()->route('checkout.thankyou_page')->with('success', __('Order successfully created!'));
            }
        }
    ...
    public function getTaxAmount(Request $request)
    {
        ...
        $data['tax_amount'] = tax_amount(Cart::subtotal());
        $data['tax_show'] =   currencyConverter(tax_amount(Cart::subtotal()));
        ...
        $data['total_cost'] = Cart::subtotal() + delivery_charge($request->country) + tax_amount(Cart::subtotal()) - Session::get('CouponAmount');
        $data['total_cost_curr'] =  currencyConverter(Cart::subtotal() + delivery_charge($request->country) + tax_amount(Cart::subtotal()) - Session::get('CouponAmount'));
        $data['success'] = true;
        return $data;
    }
