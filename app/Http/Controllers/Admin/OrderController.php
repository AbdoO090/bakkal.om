...
class OrderController extends Controller
{
    public function orders(Request $request, $status)
    {
        if ($request->ajax()) {
            if ($status == 'pending') {
                $data = Order::where('Order_Status', ORDER_PENDING)->whereIn('Payment_Status', [PAYMENT_SUCCESS,PAYMENT_PENDING]);
            } elseif ($status == 'processing') {
                $data = Order::where('Order_Status', ORDER_PROCESSING)->whereIn('Payment_Status', [PAYMENT_SUCCESS,PAYMENT_PENDING]);
            } elseif ($status == 'shipped') {
                $data = Order::where('Order_Status', ORDER_SHIPPED)->whereIn('Payment_Status', [PAYMENT_SUCCESS,PAYMENT_PENDING]);
            } elseif ($status == 'delivered') {
                $data = Order::where('Order_Status', ORDER_DELIVERED)->whereIn('Payment_Status', [PAYMENT_SUCCESS,PAYMENT_PENDING]);
            } elseif ($status == 'returned') {
                $data = Order::where('Order_Status', ORDER_RETURN)->whereIn('Payment_Status', [PAYMENT_SUCCESS,PAYMENT_PENDING]);
            } elseif ($status == 'all') {
                $data = Order::query()->whereIn('Payment_Status', [PAYMENT_SUCCESS,PAYMENT_PENDING]);
            }
    ...
    {
        if ($request->ajax()) {
            $data = Order::where('Payment_Method', '!=', COD)->whereIn('Payment_Status', [PAYMENT_SUCCESS,PAYMENT_PENDING]);
            return DataTables::of($data)
                ->addIndexColumn()
