...
if (!function_exists('Category_Des_Icon')) {
    ...
    {
        return Category::where('en_Description', '!=', null)->Where('Status', 1)->get();
    }
}
...
if (!function_exists('conversion_rate')) {
    function conversion_rate($iso)
    {
        $currency = Currency::where('iso', $iso)->first();
        if ($currency) {
            return $currency->convert_from_usd;
        }
        return 0;
    }
}
...
if (!function_exists('tax_amount')) {
    ...
    {
        $tax = 0;
        $tax_percentage = Tax::where('status', ACTIVE)->first();
...
if (!function_exists('delivery_charge')) {
    function delivery_charge($country = null)
    {
        $dc = 0;
        if ($country != null) {
            $delivery_charge = Cities::where('name_en', $country)->where('status', ACTIVE)->first();
...
if (!function_exists('orderCount')) {
    ...
    {
        if ($status != null) {
            return Order::where('Order_Status', $status)->whereIn('Payment_Status', [PAYMENT_SUCCESS,PAYMENT_PENDING])->count();
        } else {
            return Order::whereIn('Payment_Status', [PAYMENT_SUCCESS,PAYMENT_PENDING])->count();
        }
    }
}
