@extends('admin.master', ['menu' => 'delivery_charge', 'submenu' => 'states_list'])
@section('title', isset($title) ? $title : '')
@section('content')
    <div id="table-url" data-url="{{ route('admin.statesList') }}"></div>
    <div class="row">
        <div class="col-md-12">
            <div class="breadcrumb__content">
                <div class="breadcrumb__content__left">
                    <div class="breadcrumb__title">
                        <h2>{{ __('Governorate List') }}</h2>
                    </div>
                </div>
                <div class="breadcrumb__content__right">
                    <nav aria-label="breadcrumb">
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">{{ __('Governorate List') }}</li>
                        </ul>
                    </nav>
                </div>
            </div>
    ...
    <div class="modal fade" id="createModal1" tabindex="-1" role="dialog" aria-labelledby="createModalTitle1"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form enctype="multipart/form-data" method="POST" action="{{ route('admin.States_store') }}">
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <form enctype="multipart/form-data" method="POST"
                        action="{{ route('admin.States_update', encrypt($dc->id)) }}">
                        <div class="modal-body">
                            @csrf
                            <div class="input__group mb-25">
                                <label for="name_en">{{ __('English name') }}</label>
                                <input type="text" id="name_en" name="name_en" placeholder="{{ __('English name') }}"
                                    value="{{ $dc->name_en }}" required>
                            </div>
                            <div class="input__group mb-25">
                                <label for="name_ar">{{ __('Arabic name') }}</label>
                                <input type="text" id="name_ar" name="name_ar" placeholder="{{ __('Arabic name') }}"
                                    value="{{ $dc->name_ar }}" required>
                            </div>
                            <div class="input__group mb-25">
    ...
    @push('post_scripts')
        <script src="{{ asset('backend/js/admin/datatables/delivery-charge/States.js') }}"></script>
    @endpush
@endsection
